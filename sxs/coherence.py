...
def get_RongowaiWAFRMSD(ddm,
    ...
    delay_axis_chips[: delay_peak_index] = [x - delay_axis_chips[delay_peak_index]
                                            for x in delay_axis_chips[: delay_peak_index]]
    delay_axis_chips[delay_peak_index+1:] = [x - delay_axis_chips[delay_peak_index]
    ...
    span_over_which_to_compute_rmsd = [x for x in span_over_which_to_compute_rmsd if 0 <= x < len(power_vs_delay)]
...
def coherence_detection(L0, L1, rx_pos_lla):
    ...
    for channel in range(L0.J_2):  # channel-by-channel processing
        ...
        for z in range(len(altitude)):
            ...
            if power_vs_delay_snr_db[i, channel] < power_vs_delay_snr_min or altitude[z] < aircraft_alt_min:
                coherence_state[i, channel] = 5  # Uncertain coherence state
            elif coherence_metric[i, channel] <= 0.25:
                coherence_state[i, channel] = 1  # With high confidence, state is dominantly coherent
                ...
                coherence_state[i, channel] = 4  # With high confidence, state is dominantly incoherent

    L1.postCal['coherence_metric'] = np.concatenate((coherence_metric, coherence_metric), axis=1)
    L1.postCal['coherence_state'] = np.concatenate((coherence_state, coherence_state), axis=1)
