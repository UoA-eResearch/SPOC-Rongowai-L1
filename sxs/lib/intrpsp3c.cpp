...
void SP3cFile::setPathFilenameMode(char *inputFilePath)
{
  ...
  if( ! fileStream )
    printf("Unable to open sp3 file.\n");
}
...
int SP3cFile::readHeader()
{
  ...
  while( getline(fileStream, inputRec, '\n') )
  {
    ...
    if( recType[0] == '#' && recType[1] != '#' )
    {
      ...
      {
        printf("This program reads only SP3 files with format-mode: aP, bP, or cP ");
        return -1;
      }
      modeFlag = inputRec[2];
  ...
  {
    printf("Error reading the PRNs from the header for SP3 file");
    return 1;
  }
  else
    return 0;
 ...
 */
  ...
  if( sumZeroes > 1 )
  {
    ...
    printf("WARNING: There are data values equal to zero ");
    for( k = 0; k < NData; k++)
    {
  ...
  if( sum999999 > 0 )
  {
    cerr.setf(ios::fixed);
    cerr.setf(ios::showpoint);
    printf("WARNING: There are data values equal to 999999 ");
    for( k = 0; k < NData; k++)
    {
      cerr << k + 1 << " " << setw(20) << setprecision(15) << X[k] << "  "
        << setw(15) << setprecision(6) << Y[k] << endl;
